...
int xsocket(int domain, int type, int protocol)
{
   ...
   {
      RARCH_ERR("Smart-Server: Error on socket()\n");
      exit(EXIT_FAILURE);
   }
...
int xbind(int socket, const struct sockaddr *address, socklen_t address_len)
{
   ...
   {
      RARCH_ERR("Smart-Server: Error on bind()\n");
      close(socket);
      exit(EXIT_FAILURE);
   }
...
int xlisten(int socket, int backlog)
{
   ...
   {
      RARCH_ERR("Smart-Server: Error on listen\n");
      close(socket);
      exit(EXIT_FAILURE);
   }
...
int xaccept(int socket, struct sockaddr * address, socklen_t * address_len)
{
   ...
   {
      RARCH_ERR("Smart-Server: Error on accept()\n");
      exit(EXIT_FAILURE);
   }
...
int xselect(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout)
{
   ...
   if (ret == -1)
   {
      RARCH_ERR("Smart-Server: Error on select()\n");
      exit(EXIT_FAILURE);
   }
